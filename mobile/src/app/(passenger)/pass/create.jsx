...
export default function CreatePass() {
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: BG }} contentContainerStyle={{ paddingTop: insets.top + 12, padding: 18, paddingBottom: 80 }}>
    ...
    {step === 4 ? <View style={{ marginTop: 28, alignItems: "center" }}><View style={{ width: 70, height: 70, borderRadius: 24, backgroundColor: "#15351F", alignItems: "center", justifyContent: "center" }}><Check color="#22C55E" size={36} /></View><Text style={{ color: TEXT, fontSize: 22, fontWeight: "900", marginTop: 18 }}>{result?.paymentLink ? "Complete payment" : "Pass request created"}</Text><Text style={{ color: MUTED, textAlign: "center", marginTop: 8 }}>{result?.paymentLink ? "Use the secure Razorpay payment link to activate driver matching. We also sent it by WhatsApp or SMS." : "Payments are not configured in this environment. Your request is saved for setup testing."}</Text>{result?.paymentLink ? <TouchableOpacity onPress={() => Linking.openURL(result.paymentLink)} style={{ marginTop: 20, backgroundColor: GOLD, padding: 15, borderRadius: 14, width: "100%", alignItems: "center" }}><Text style={{ color: "#111", fontWeight: "900" }}>Pay Securely</Text></TouchableOpacity> : null}<TouchableOpacity onPress={() => router.replace("/(passenger)/pass")} style={{ marginTop: 12, borderWidth: 1, borderColor: GOLD, padding: 15, borderRadius: 14, width: "100%", alignItems: "center" }}><Text style={{ color: GOLD, fontWeight: "900" }}>View My Pass</Text></TouchableOpacity></View> : null}
