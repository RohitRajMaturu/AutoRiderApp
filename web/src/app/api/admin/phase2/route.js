...
export async function GET(request) {
  try {
    ...
    const [drivers, passes, institutions, trips, slaEvents, funnel] =
      await Promise.all([
        sql`
      SELECT d.id, u.name, d.is_online, d.sla_score,
        COALESCE((
          SELECT round(avg(rating.driver_rating)::numeric, 1)
          FROM rides rating
          WHERE rating.driver_id = d.id AND rating.driver_rating IS NOT NULL
        ), 0) AS rating,
    ...
    console.error("GET /api/admin/phase2 error:", error);
    return Response.json(
      {
        error: "Unable to load Phase 2 operations",
        code: error?.code || "PHASE2_ADMIN_LOAD_FAILED",
